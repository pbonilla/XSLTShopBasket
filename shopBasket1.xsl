...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="productsDoc" select="document('products.xml')"/>

    <xsl:template match="/">
        <!--<xsl:result-document href="shopTest.xml" method="xml">-->
    ...
    <xsl:template match="updates" name="test">
        ...
        <xsl:variable name="prodCounter" select="count($productsDoc/*/*)"/>
        <xsl:choose>
                ...
                <xsl:if test="count(child::*[@ID=$idProd])>0">
                    <xsl:choose>
                        <xsl:when test="child::*[@ID=$idProd and (name()='add' or name()='update')]">
                            ...
                            <xsl:apply-templates select="$productsDoc/*/*[@prd:ID = $idProd]">
